<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="projectsort.aspx.cs" Inherits="ETS2.WebApp.UI.PMUI.projectsort" %>
...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $.post("/JsonFactory/ProductHandler.ashx?oper=projectlist", { comid: comid, projectstate: "1" }, function (data2) {
                data2 = eval("(" + data2 + ")");
                if (data2.type == 1) {
                    $.prompt("列表获取出现问题");
                    return;
                ...
                if (data2.type == 100) {
                    ...
                    for (var i = 0; i < data2.totalCount; i++) {
                        str += '<li id="' + data2.msg[i].Id + '"><a href="#">产品ID:' + data2.msg[i].Id + '--—产品名称:' + data2.msg[i].Projectname + '</a></li>';
                    }
                    $("#myList").append(str);
                }
            })
            ...
            $("#btnsort").click(function () {
                if ($.cookie("myCookie")) {
                    ...
                    if (ids == null) {
                        window.open("ProjectList.aspx", target = "_self");
                    } else {
                        $.post("/JsonFactory/ProductHandler.ashx?oper=projectMenusort", { menuids: ids }, function (data2) {
                            data2 = eval("(" + data2 + ")");
                            if (data2.type == 1) {
                                alert("操作出现问题");
                                return;
...
<body>
    ...
    <a href="ProjectList.aspx">返回列表</a>
