<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="/UI/Etown.Master" CodeBehind="AgentBackCode.aspx.cs"
    Inherits="ETS2.WebApp.UI.PMUI.AgentBackCode" %>
...
<asp:Content ID="content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            function SearchList(pageindex) {
                ...
                $.ajax({
                    ...
                    data: { comid: comid, pageindex: pageindex, pagesize: pageSize, key: key, order_state: order_state, ordertype: 3 },
                    async: false,
            ...
            $("#AgentBackCodeToExcel").click(function () {
                window.open("/excel/DownExcel.aspx?oper=agentbackcodetoexcel&comid=" + comid + "&order_state=" + $("#order_state").trimVal() + "&servertype=" + $("#sel_servertype").trimVal() + "&key=" + $("#key").trimVal() + "&beginDate=" + $("#startime").trimVal() + "&endDate=" + $("#endtime").trimVal() + "&userid=" + userid + "&ordertype=3", target = "_blank");
            })
        ...
        //录入糯米交易号
        function showNuomiDealid(id, proname, Nuomi_dealid) {
            $("#Span1").text("录入糯米交易号(订单:" + id + ")");
            $("#hid_orderid").val(id);
            $("#Span2").text(proname);

            $("#Textarea1").val(Nuomi_dealid);
            $("#div_nuomi").show();
        }
        function InsNuomiDealid() {
            var orderid = $("#hid_orderid").val();
            var nuomidealid = $("#Textarea1").val();
            $.post("/JsonFactory/OrderHandler.ashx?oper=insnuomi_dealid", { orderid: orderid, nuomidealid: nuomidealid }, function (data) {
                data = eval("(" + data + ")");
                if (data.type == 1) { alert("录入糯米交易号出错"); return; }
                if (data.type == 100) {
                    alert("录入糯米交易号成功");
                    $("#div_nuomi").hide();
                    return;
...
<asp:Content ID="content2" ContentPlaceHolderID="body" runat="server">
    <div id="settings" class="view main">
        <%--  <div id="secondary-tabs" class="navsetting ">
            <ul>
                ...
                <li class="on"><a href="AgentBackCode.aspx" onfocus="this.blur()" target=""><span>导码订单</span></a></li>
        ...
        <div id="setting-home" class="vis-zone  mail-list">
            <div class="inner">
                <h3>
                    订单管理</h3>
                <div style="float: right; padding-right: 110px; width: 800px;">
                    <label>
                        请输入(手机，姓名，订单号，电子码)
                ...
                <div style="float: right; padding-right: 220px; width: 800px; text-align: right;">
                    <label>
                        <input type="button" value="导出到Excel" style="width: 120px; height: 26px;" id="AgentBackCodeToExcel">
                    </label>
                </div>
    ...
    <script type="text/x-jquery-tmpl" id="ProductItemEdit">   
                    <tr class="fontcolor">
                       ...
                       <td valign="top">
                        ...
                        <td valign="top">
                            <p align="left">
                                <label title="${Agent_company}"> ${Agent_company}</label>
                                {{if Server_type==3}}<br><label title="${ChangeDateFormat(U_traveldate)}">${ChangeDateFormat(U_traveldate)}</label>{{/if}}
                        ...
                        <td valign="top">
                            <p align="center">
                               {{if Agentid!="0"}} 分销 {{if Warrant_type=="1"}}(出票扣款){{else}}(验码扣款){{/if}} {{else}}{{if Openid==""}}网站{{else}}微信{{/if}}{{/if}}
                            </p>
                        </td>
                        <td valign="top">
                            <p align="center">
                            ...
                            {{else}}
                                   {{if Agentid=="0"}}
                                        ...
                                        {{if Order_state==4}} 
                                            ...
                                            {{else}}
                                                <a href="/Agent/DownExcel.aspx?comid=${Comid}&agentid=${Agentid}&orderid=${Id}&md5info=${Returnmd5}" style="font-size: 14px; font-weight: bold;color:#333933">下载电子码</a> <a href="AgentBackCodeCount.aspx?agentid=${Agentid}&orderid=${Id}">电子码管理</a>
                                            {{/if}}
                                        {{/if}}
                                   {{else}}
                                        {{if Order_state==4}} 
                                   ...
                                   {{if Order_state==16}}订单退票,${Ticketinfo}{{/if}}
                                   
                             {{/if}} 
                             {{/if}}
                             {{if AgentType=="2"}}
                                 <input type="button" onclick="showNuomiDealid('${Id}','${Proname}','${Nuomi_dealid}')"   value="糯米交易号"/>
                             {{/if}}
                                </p>
                        </td>
                    </tr>
    ...
    <div id="div_nuomi" style="background-color: #ffffff; border: 2px solid #5984bb;
        margin: 0px auto; width: auto; height: auto; display: none; left: 20%; position: absolute;
        top: 20%;">
        <table width="100%" border="0" cellpadding="10" cellspacing="1" bgcolor="#999999"
            style="padding: 5px;">
            <tr>
                <td height="42" colspan="2" bgcolor="#C1D9F3">
                    <span style="padding-left: 10px; font-size: 12px; font-weight: bold" id="Span1">
                    </span>
                </td>
            </tr>
            <tr>
                <td height="20" bgcolor="#C1D9F3" class="tdHead" style="padding-left: 10px; font-size: 12px;">
                    产 品：<span id="Span2"></span>
                </td>
            </tr>
            <tr>
                <td height="20" bgcolor="#E7F0FA" class="tdHead" style="padding-left: 10px; font-size: 12px;">
                    糯米交易号:<input name="Input" class="dataNum dataIcon" id="Textarea1" value="">
                </td>
            </tr>
            <tr>
                <td height="38" colspan="2" align="center" bgcolor="#FFFFFF" class="tdHead">
                    <input type="hidden" id="hid_orderid" value="" />
                    <input id="Button1" name="w_Enter" type="button" class="formButton" value="  确   认  "
                        onclick="InsNuomiDealid()" />
                    <input name="cancel_rh" type="button" class="formButton" onclick="w_cancel();" value="  关  闭  " />
                </td>
            </tr>
        </table>
