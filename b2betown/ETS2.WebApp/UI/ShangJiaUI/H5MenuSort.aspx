<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="H5MenuSort.aspx.cs" Inherits="ETS2.WebApp.UI.ShangJiaUI.H5MenuSort" %>
...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $.post("/JsonFactory/DirectSellHandler.ashx?oper=getmenulist", { comid: $("#hid_comid").val(), pageindex:1,pagesize:100}, function (data2) {
                data2 = eval("(" + data2 + ")");
                if (data2.type == 1) {
                    $.prompt("获取出现问题");
                    return;
                ...
                if ($.cookie("myCookie")) {
                    ...
                    $.post("/JsonFactory/DirectSellHandler.ashx?oper=menuSort", { ids: ids }, function (data2) {
                        ...
                        if (data2.type == 100) {
                            ...
                            window.open("H5SetMenu.aspx", target = "_self");
                        }
                    })
                } else {
                    alert("请先拖拽进行排序");
