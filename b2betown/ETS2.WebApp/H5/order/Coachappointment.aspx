<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Coachappointment.aspx.cs" Inherits="ETS2.WebApp.H5.order.Coachappointment" %>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            function SearchList(pageindex) {
                ...
                $.post("/JsonFactory/OrderHandler.ashx?oper=getjiaolianyuyueorder", { pageindex: pageindex, comid: comid, channelid: channelid, startime: startime }, function (data) {
            ...
            $(document).ready(function () {
                ...
                $("#calendar").datepicker({
                    ...
                    beforeShowDay: function (date) {
                        ...
                        if (result) {
                            return [true, "hasCourse", "<p>" + dayprice + "笔</p>"];

                        } else {
                            return [false, "noCourse", '无预订'];
                        }
                    }
                });
            });
    ...
    <div class="container ">
                <div class="content"  style="min-height: 288px;">
					<div class="tabber  tabber-n3 tabber-double-11 clearfix">
						...
						<a class="active" href="Coachappointment.aspx">我的预约</a>
...
<div class="header">
 ...
 <hr class="margin-0 left-10">
    ...
    <div class="opt-btn">
                        ...
                        {{if Order_state==2}}
                        {{if Server_type==1}}
                            <a href="#" class="btn btn-in-order-list" >已支付</a>
                        {{else}}
                            <a href="#" class="btn btn-in-order-list" >已支付</a>
                        {{/if}}
                        {{/if}}
                        ...
                        {{else}}
                            ...
                            {{else}}
                              {{if Server_type==12}}
                                    <a href="#" class="btn btn-in-order-list" >预订提交成功</a>
                              {{else}}
                                    {{if Server_type==13}}
                                    <a href="#" class="btn btn-in-order-list" >预约成功</a>
                                    {{else}}
                        ...
                        {{if Order_state==8}}
                        {{if Server_type==1|| Server_type==3  }}
                            <a href="#" class="btn btn-in-order-list" >已验证</a>
                        {{else}}
                            {{if Server_type==11}}
                                <a href="#" class="btn btn-in-order-list" >已发货 ${Expresscom}${Expresscode}</a>
                            {{else}}
    ...
    <input id="hid_startime" type="hidden" value="<%=startime %>" />

    <input id="hidLeavingDate" type="hidden" value="<%=datelist %>" />
    <input id="hidLinePrice" type="hidden" value="<%=numlist %>" />
