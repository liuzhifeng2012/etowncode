<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="ETS2.WebApp.H5.order.List" %>
                ...
                if ($(document).height() <= totalheight) {
                    if (stop == true) {
                        ...
                        $.ajax({
                            ...
                            data: { comid: comid, pageindex: pageindex, pagesize: pageSize, projectid: projectid, key: $("#key").val() },
                            async: false,
...
<body class=" ">
    ...
    <div class="container ">
        ...
        function DRIVINGdaohang() {
            ...
            ss.routeCall(start, end, opts);
                ...
                <!-- 商品区域 -->
                <!-- 展现类型判断 -->
                <ul id="list" class="sc-goods-list clearfix list size-3">
                </ul>
            </div>
    ...
    <script type="text/x-jquery-tmpl" id="ProductItemEdit"> 
                ...
                <li class="goods-card goods-list normal">

                    {{if (Server_type==2 || Server_type==8)}}
                     <a title="${Project_name}" data-goods-id="6427318" target="_blank" class="link js-goods clearfix" href="/h5/linedetail.aspx?lineid=${Id}">
                    {{else}}
                        {{if Server_type==9}}
                        ...
                        {{else}}
				                <a title="${Project_name}" data-goods-id="6427318" target="_blank" class="link js-goods clearfix" href="pro.aspx?id=${Id}">
                        {{/if}}
                    {{/if}}

                   
                       <div class="photo-block">
                           <img data-src="${Imgurl}" class="goods-photo js-goods-lazy" style="display: block;" src="${Imgurl}">
                       </div>
                       <div class="info">
                            <p class="goods-title">${Pro_name}</p>
                            <p class="goods-price"><em>￥${Advise_price}</em></p>
                               
                               <div class="goods-buy btn1"></div>
                            </div>              
                    </a>
                            <div class="goods-buy btn0"> </div> 
                     
				           <div onclick="suborder('${Id}','${Pro_name}','${Imgurl}','${Advise_price}','${Server_type}');" data-price="{Advise_price}" data-title="${Project_name}" data-id="${Id}" data-buyway="1" class="js-goods-buy buy-response"></div>
