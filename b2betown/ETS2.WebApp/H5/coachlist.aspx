<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="coachlist.aspx.cs" Inherits="ETS2.WebApp.H5.coachlist" %>
                ...
                if ($(document).height() <= totalheight) {
                    if (stop == true) {
                        ...
                        $.ajax({
                            ...
                            data: { comid: comid, pageindex: pageindex, pagesize: pageSize, channelcompanyid: 0, key: "", openid: $("#hid_openid").val(), usern: $("#usern").val(), usere: $("#usere").val(), isonlycoachlist: 1 },
                            async: false,
...
<div style="" class="container view-animate ng-scope" ng-view="" onload="viewloadHandler()" autoscroll="">
 ...
 <script type="text/x-jquery-tmpl" id="ProductItemEdit"> 
					...
					<li class="clearfix ng-scope" ng-repeat="art in artis">
                        ...
                        {{if bindingproid!=0}}
                        <a href="order/pro.aspx?MasterId=${Id}&id=${bindingproid}">
                        {{else}}
                         <%if(come!=""){%>
                          <a href="order/pro.aspx?MasterId=${Id}&id=<%=come%>">
                         <%
                        }else{ %>
                        ...
                        {{/if}}
							...
							<ul class="title">
                                ...
                                {{if Workingyears!=0}}
                                    ...
                                    {{/if}}
                                    {{if Workingyears==6}}从业6年
                                    {{/if}}
                                    {{if Workingyears==7}}从业7年
                                    {{/if}}
                                    {{if Workingyears==8}}从业8年
                                    {{/if}}
                                    {{if Workingyears==9}}从业9年
                                    {{/if}}
                                    {{if Workingyears==10}}从业10年
                                    {{/if}}
                                    {{if Workingyears==11}}从业11年
                                    {{/if}}
                                    {{if Workingyears==12}}从业12年
                                    {{/if}}
                                    {{if Workingyears==13}}从业13年
                                    {{/if}}
                                    {{if Workingyears==14}}从业14年
                                    {{/if}}
                                    {{if Workingyears==15}}从业15年
                                    {{/if}}
                                    {{if Workingyears==16}}从业16年
                                    {{/if}}
                                    {{if Workingyears==17}}从业17年
                                    {{/if}}
                                    {{if Workingyears==18}}从业18年
                                    {{/if}}
                                    {{if Workingyears==19}}从业19年
                                    {{/if}}
                                    {{if Workingyears==20}}从业20年+
                                    {{/if}}
                                    {{/if}}
                                </span>
                                </li>
