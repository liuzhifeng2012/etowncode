...
$("#last_mon1").click(function () { prevMonth1() });
$("#next_mon1").click(function () { nextMonth1() });
function prevMonth1() {
    if ((month - 1) < 0) {
        month = 11; year--
    } else {
...
function thisMonth1(a) {
    obj = a; year = today.year; month = today.month; fillBox1(obj)
}
function fillBox1(j) {
    var s = "", g = 0;
    for (var k = 0; k < 6; k++) {
        s += "<tr>"; for (var p = 0; p < 7; p++) {
            if (p == 0) { s += '<td class="calBox sun" id="calBoxl1' + g + '"></td>' } else {
                if (p == 6) { s += '<td class="calBox sat" id="calBoxl1' + g + '"></td>' } else {
                    s += '<td class="calBox" id="calBoxl1' + g + '"></td>'
                }
            } g++
        }
        s += "</tr>"
    }
    $("#calendar1").html('<table id="calTable1"><tr><th class="week_day">周日</th><th class="week_day">周一</th><th class="week_day">周二</th><th class="week_day">周三</th><th class="week_day">周四</th><th class="week_day">周五</th><th class="week_day">周六</th></tr>' + s + "</table>");
    updatedateInfo1();
    $("td.calBox").empty();
    var t = 1;
    ...
    for (var k = m; k <= b; k++) {
        ...
        if (t < q || !timeContrast(u + "-" + e + "-" + o, year + "-" + f + "-" + r)) {
            $("#calBoxl1" + k).html("<div class='week_day'>" + l + "<span class='week_day ui-state-default'>" + r + "</span></div>")
        } else {
            $("#calBoxl1" + k).html("<div class='week_day'>" + l + "<a href='javascript:;' class='week_day' id='" + year + "-" + f + "-" + r + "'><span class='week_day'>" + t + "</span></a></div>")
        }
        //        } 
        t++
    }
    ...
    $("#holidayList1").html(l);
    $("#calTable1 a").click(function () {
        var v = $(this).attr("id");
        if (v != null) {
            SearchList(1, 3, $("#hid_proid").val(), $("#indate").html(), v);
            $("#goBacktime1").click();
            $("#outdate").html(v);
        }
        else {
...
function updatedateInfo1() {
    $("#dateInfo1").html(year + "年" + (month + 1) + "月");
    if (month == 0) { $("#last_mon1 span").html("12月") } else { $("#last_mon1 span").html(month + "月") }
    if (month == 11) { $("#next_mon1 span").html("1月") } else { $("#next_mon1 span").html((month + 2) + "月") }
}
