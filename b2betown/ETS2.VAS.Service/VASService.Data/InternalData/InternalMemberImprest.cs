...
{
     public class InternalMemberImprest
    {
         
         private SqlHelper sqlHelper;
         public InternalMemberImprest(SqlHelper sqlHelper)
        {
            this.sqlHelper = sqlHelper;
        }

         #region 积分插入数据库
         private static readonly string SQLInsertOrUpdate = "usp_InsertOrUpdateImprest";
         internal int InsertOrUpdate(Member_Imprest model)
         {
         ...
         /// <returns></returns>
         internal List<Member_Imprest> ReadImprest(int id, int comid, out int totalcount)
         {
           int readi = 0;
      ...
      ,orderid
      ,orderName
        FROM [EtownDB].[dbo].[Member_Imprest_Log] as a where a.com_ID=@Comid and a.mid=@Id order by id desc";
