...
{
    public class InternalTaobao_modified_noticelog
    {
         public SqlHelper sqlHelper;
         public InternalTaobao_modified_noticelog(SqlHelper sqlHelper)
        {
            this.sqlHelper = sqlHelper;
        }

         internal int Editnoticelog(Model.Taobao_modified_noticelog log)
         {
             if (log.id == 0)
             {
                 string sql = @"INSERT INTO  [taobao_modified_noticelog]
           ([timestamp]
           ,[sign]
           ,[order_id]
           ,[mobile]
...
,@left_num
           ...
           ,@md5_mobile);select @@identity;";
             ...
             {
                 string sql = @"UPDATE  [taobao_modified_noticelog]
...
,left_num=@left_num
 ...
 WHERE id=@id";
         ...
         {
             string sql = "select count(1) from taobao_modified_noticelog where token='"+token+"'";
             var cmd = sqlHelper.PrepareTextSqlCommand(sql);
             object o = cmd.ExecuteScalar();
             return int.Parse(o.ToString());
         }
